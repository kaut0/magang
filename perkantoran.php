<?php
    include ('include/header.php');
?>
<style type="text/css">
.foto img {
    max-height: 300px;
    max-width: 300px;
}

body {
    background-color: whitesmoke;
    font-family: new time roman;
}

hr.solid {
    border-top: 18px solid #bbb;
}

.rapi {
    text-align: justify;
}
</style>

<main role="main">
    <section class="jumbotron text-left text-white gambar">
        <div class="container">
            <h1>ADMINISTRASI PERKANTORAN</h1>
        </div>
    </section>
    <br />
    <div class="container mb">
        <div class="container">
            <div class="row">
                <div class="card">
                    <div class="card-header">
                        <h2>MANAJEMEN PERKANTORAN</h2>
                    </div>
                    <div class="card-body rapi">
                        <ul>
                            <li>
                                Program ini dirancang untuk menciptakan profesional muda yang memiliki kemampuan
                                memadukan ketrampilan dibidang bahasa inggris, ketrampilan dalam korespondensi bisnis,
                                presentasi, mengadministrasikan dokumen dan memiliki kepribadian seorang sekretaris,
                                yang dilandasi dengan jiwa entrepreneur dan rasa percaya diri yang tinggi.
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="container mb">
        <div class="container">
            <div class="row">
                <div class="card">
                    <div class="card-header">
                        <h2>Kompetensi Lulusan</h2>
                    </div>
                    <div class="card-body rapi">
                        <li>Mampu berkomonikasi dalam bahasa inggris</li>
                        <li>Mampu membuat surat dan proposal bisnis baik dalam bahasa Indonesia serta mampu
                            mengadministrasikannya</li>
                        <li>Mampu mengadministrasikan seluruh kegiatan aktifitas perkantoran, mengelola system
                            prosedur kantor dan mempunyai wawasan global</li>
                        <li>Mampu mengelola, menyimpan dan menemukan kembali dokumen dengan menggunakan system
                            kearsipan yang berlaku di organisasi</li>
                        <li>Mampu menggunakan teknologi informasi dan fasilitas komputer untuk memecahkan masalah bisnis
                            secara praktis</li>
                        <li>Memiliki jiwa entrepreneur</li>
                        <li>Mampu bersikap baik, berpenampilan professional dan memiliki kepercayaan diri tinggi</li>
                    </div>
                </div>
                <div class="row">
                    <div class="container">
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-body">
                                    <table class="table table-bordered table-hover table-striped">
                                        <thead>
                                            <tr>
                                                <th>NO.</th>
                                                <th>MATERI</th>
                                                <th>SKS</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td></td>
                                                <td>SEMESTER 1</td>
                                                <td></td>
                                            </tr>
                                            <tr>
                                                <td>1</td>
                                                <td>English 1</td>
                                                <td>4 sks</td>
                                            </tr>
                                            <tr>
                                                <td>2</td>
                                                <td>Computer For Office 1</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>3</td>
                                                <td>Business Corespondence</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>4</td>
                                                <td>Typing Tutor / Keyboarding</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>5</td>
                                                <td>Personality Deplopment</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>6</td>
                                                <td>Filling Management</td>
                                                <td>4 sks</td>
                                            </tr>
                                            <tr>
                                                <td>7</td>
                                                <td>Secretarial Duties</td>
                                                <td>4 sks</td>
                                            </tr>
                                            <tr>
                                                <td></td>
                                                <td>Total SKS</td>
                                                <td>20 sks</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
                <div class="row">
                    <div class="container">
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-body">
                                    <table class="table table-bordered table-hover table-striped">
                                        <thead>
                                            <tr>
                                                <th>NO.</th>
                                                <th>MATERI</th>
                                                <th>SKS</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td></td>
                                                <td>SEMESTER 2</td>
                                                <td></td>
                                            </tr>
                                            <tr>
                                                <td>1</td>
                                                <td>English 2</td>
                                                <td>4 sks</td>
                                            </tr>
                                            <tr>
                                                <td>2</td>
                                                <td>Computer For Office 2</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>3</td>
                                                <td>Modern Offce Administration</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>4</td>
                                                <td>Business Coresspondences</td>
                                                <td>4 sks</td>
                                            </tr>
                                            <tr>
                                                <td>5</td>
                                                <td>Financial Administration & Banking Procedure</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>6</td>
                                                <td>Service Excellent</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td>7</td>
                                                <td>Enterpreneur</td>
                                                <td>2 sks</td>
                                            </tr>
                                            <tr>
                                                <td></td>
                                                <td>Total SKS</td>
                                                <td>20 sks</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

</html>

<?php
include ('include/footer.php');
?>